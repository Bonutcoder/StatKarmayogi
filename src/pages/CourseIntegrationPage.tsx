...
import { triggerIGotSync } from "../services/api";
...
export default function CourseIntegrationPage({
  ...
  const handleSync = async () => {
    setSyncing(true);
    const result = await triggerIGotSync();
    setSyncing(false);
    ...
    <div style={{ flex: 1, overflow: "auto", padding: 28 }}>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <IntegrationBadge status={providerMode === "LIVE_IGOT" ? "iGOT" : "LOCAL"} />
        </div>
      </div>
      ...
      <div style={{ maxWidth: 840 }}>
        ...
        <div style={{ background: "#fff", border: `1px solid ${border}`, padding: 24, marginBottom: 20 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: muted, fontFamily: "'Space Grotesk', 'Outfit', sans-serif", marginBottom: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                {providerMode === "MOCK" && (
                  <span style={{ fontSize: 10, color: coral, fontWeight: 800, fontFamily: "'Space Grotesk', 'Outfit', sans-serif" }}>
                    ACTIVE
                  </span>
                )}
              </div>
              <div style={{ fontSize: 12, color: muted, marginTop: 6, lineHeight: 1.5 }}>
                Local course catalogue used when a live iGOT connection is unavailable.
              </div>
            </div>
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                {providerMode === "LIVE_IGOT" && (
                  <span style={{ fontSize: 10, color: emerald, fontWeight: 800, fontFamily: "'Space Grotesk', 'Outfit', sans-serif" }}>
                    CONNECTED
                  </span>
                )}
              </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px", background: panel, border: `1px solid ${border}` }}>
            <div>
              ...
              <div style={{ fontSize: 11, color: muted, marginTop: 2 }}>
                Last synced: <span style={{ fontFamily: "'Space Grotesk', 'Outfit', sans-serif", color: slate }}>{lastSyncTime}</span>
              </div>
            </div>
            <button
              disabled={syncing}
        ...
        <div style={{ background: "#fff", border: `1px solid ${border}`, padding: 24 }}>
          ...
          <div style={{ fontFamily: "'Space Grotesk', 'Outfit', sans-serif", fontSize: 12, background: panel, padding: 16, border: `1px solid ${border}`, lineHeight: 1.7, color: slate }}>
